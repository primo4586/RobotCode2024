...
public class LedsSubsystem extends SubsystemBase {
  ...
  private static LedsSubsystem instance = new LedsSubsystem();
  ...
  public LedsSubsystem() {
    m_led = new AddressableLED(LedsConstants.LedsLength);
  ...
  // should make rainbow when note in feeder and blue/orange when no note in
  // feeder
  public void GameLeds(int delay) {
    while (true) {
      while (TakeFeedSubsystem.getInstance().getOpticSensorValue()) {
        for (int i = 0; i < LedsConstants.LedsLength; i++) {
          final int hue = (counter + (i * 180 / LedsConstants.LedsLength)) % 180;
          m_ledBuffer.setHSV(i, hue, 255, 128);
        }
